...
use ucma_scope::canonicalize::{canonicalize, canonicalize_url};
...
async fn test_e2e_path_prefix_filtering() {
    ...
    let policy = ScopePolicy::builder()
        ...
        .allow_path_prefix("/api/v1/")
        .deny_path_prefix("/api/v1/admin")
...
async fn test_e2e_url_canonicalization() {
    ...
    assert_eq!(u3.port(), None); // Default 443 stripped in canonical form
...
async fn test_e2e_anti_ssrf_scope_authorization_denial() {
    ...
    assert!(matches!(res_meta.unwrap_err(), ScopeError::SsrfBlocked(_)));
    ...
    assert!(matches!(res_internal.unwrap_err(), ScopeError::SsrfBlocked(_)));
    ...
    assert!(matches!(res_localhost.unwrap_err(), ScopeError::SsrfBlocked(_)));
}
"""
...
async fn test_e2e_expired_token_rejected() {
    ...
    let policy = Arc::new(
        ScopePolicy::builder()
            ...
            .with_resolver(resolver)
            .with_salt([42u8; 32])
            .with_token_ttl(-10) // Negative TTL -> already expired!
            .build(),
    );
    ...
    assert!(matches!(
        send_res.unwrap_err(),
        HttpError::Scope(ScopeError::TokenExpired)
    ));
}
...
async fn test_e2e_ssrf_redirect_aborted() {
    ...
    assert!(decision_res.is_err());
    assert!(matches!(decision_res.unwrap_err(), ScopeError::SsrfBlocked(_)));
}
...
async fn test_e2e_redirect_hop_limit_enforced() {
    ...
    let decision_res = RedirectValidator::evaluate_hop(
        ...
        302,
        &headers,
        5, // Already reached max of 5 hops!
        5,
        &policy,
    )
    .await;
...
use ucma_core::ids::{ContentId, EndpointId, EvidenceId, ParameterId, RequestId, SnapshotId, TargetId};
use ucma_core::request::RawRequest;
...
fn test_e2e_content_id_determinism() {
    ...
    let id1 = ContentId::hash(data1);
    let id2 = ContentId::hash(data2);
    let id3 = ContentId::hash(data3);
...
fn test_e2e_domain_id_derivation_determinism() {
    ...
    // ParameterId
    let p1 = ParameterId::derive(e1, "user_id", "query");
    let p2 = ParameterId::derive(e1, "user_id", "query");
    assert_eq!(p1, p2);
}
...
fn test_e2e_response_snapshot_wire_hashing() {
    ...
    let snapshot = ResponseSnapshot::new(
        ...
        body.clone(),
        raw_wire.clone(),
        15_000_000, // 15ms in nanos
        false,
        Some("93.184.216.34".to_string()),
    );

    let expected_content_id = ContentId::hash(&raw_wire);
    assert_eq!(snapshot.content_id, expected_content_id);
    assert_eq!(snapshot.status_code, 200);
    assert_eq!(snapshot.latency_ms(), 15);
...
fn test_e2e_evidence_store_crud_and_querying() {
    ...
    let snapshot_id = SnapshotId::from_content(ContentId::hash(b"raw response bytes"));
    ...
    assert!(store.get(&record_id).is_some());

    // Query by target
    let by_target = store.query_by_target(&target_id);
    assert_eq!(by_target.len(), 1);

    // Query by severity
    let high_sev = store.query_by_severity(Severity::High);
    assert_eq!(high_sev.len(), 1);

    let crit_sev = store.query_by_severity(Severity::Critical);
    assert_eq!(crit_sev.len(), 0);

    // Remove
    let removed = store.remove(&record_id);
...
use std::net::IpAddr;
use std::sync::Arc;
use ucma_core::endpoint::{Endpoint, HttpMethod, PathTemplate};
use ucma_core::evidence::{EvidenceRecord, EvidenceStore, Severity};
use ucma_core::ids::{ContentId, EndpointId, ParameterId, RequestId, SnapshotId, TargetId};
use ucma_core::parameter::{EncodingChain, ParameterDefinition, ParameterLocation, ParameterType};
use ucma_core::request::RawRequest;
use ucma_core::session::SessionState;
use ucma_core::snapshot::ResponseSnapshot;
use ucma_core::target::{Target, TargetConfig};
use ucma_e2e::fixtures::{build_standard_test_policy, create_mock_dns, create_test_request};
use ucma_http::client::{HttpError, SafeHttpClient};
use ucma_http::limits::{HttpLimits, HttpLimitsBuilder};
...
fn test_tier1_target_endpoint_parameter_models() {
    let target = Target::new("https://example.com/api".to_string(), TargetConfig::default()).unwrap();
    assert_eq!(target.root_url.as_str(), "https://example.com/api");

    let endpoint = Endpoint::new(
        target.id,
        HttpMethod::Get,
        PathTemplate::parse("/users/{id}").unwrap(),
    );
    assert_eq!(endpoint.path_template.template, "/users/{id}");
    ...
    let param = ParameterDefinition::new(
        ...
        EncodingChain::identity(),
    );
    assert_eq!(param.name, "id");
    assert!(param.is_injectable());
}
...
fn test_tier1_session_state_and_token_expiry() {
    ...
    session.set_bearer_token("bearer_token_secret_xyz");
    session.set_cookie("session_id", "abc12345");

    assert_eq!(session.bearer_token.as_deref(), Some("bearer_token_secret_xyz"));
...
fn test_tier1_evidence_record_lifecycle() {
    let target_id = TargetId::derive("https://example.com");
    let snapshot_id = SnapshotId::from_content(ContentId::hash(b"test raw bytes"));
    ...
    record.add_tag("milestone:m1");
    record.add_tag("oracle:sprt");

    assert_eq!(record.tags.len(), 2);
    assert!(record.tags.contains(&"oracle:sprt".to_string()));
}
...
fn test_tier2_zero_and_empty_payload_boundaries() {
    // Empty body hashing
    let empty_hash = ContentId::hash(b"");
    assert_eq!(
        empty_hash.as_str(),
    ...
    // Empty URL rejection
    assert!(Target::new("".to_string(), TargetConfig::default()).is_err());
}
...
print("Generated all 5 E2E integration test suites successfully!")
