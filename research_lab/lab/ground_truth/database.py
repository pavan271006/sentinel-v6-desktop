"""
Database layer for Ground-Truth Vulnerability Lab.
Implements SQLite connection management and seeds test fixtures for all 8 vulnerability classes.
"""
...
class GroundTruthDatabase:
    """Thread-safe SQLite database manager for Ground-Truth Lab."""
    ...
    def _init_schema(self):
        with self.get_cursor() as cursor:
            cursor.executescript(GROUND_TRUTH_SCHEMA)
    ...
    def _seed_default_data(self, cursor: sqlite3.Cursor):
        ...
        accounts = [
            ...
            (2, "org_alpha", "bob", 100.0, "USD"),  # Seeded with $100 for race condition testing
        ...
        # 5. Workflows (wf_alpha_101 initialized with cleared tenant_context_lock simulating post-rollback state)
        workflows = [
            (
                "wf_alpha_101",
                "org_alpha",
                ...
                None  # Vulnerable state: tenant context lock dissociated after rollback!
            )
        ]
        cursor.executemany(
