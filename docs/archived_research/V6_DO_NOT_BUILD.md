# SENTINEL V6: ANTI-OVERENGINEERING REGISTER ("DO NOT BUILD")
## Authoritative Catalog of Explicitly Rejected Features, Algorithms & Architectural Anti-Patterns
**Document ID**: `SENTINEL-SPEC-V6-DNB-004`  
**Classification**: Authoritative Rejection Register & Architectural Boundary  
**Target Platform**: SENTINEL V6 Desktop Testing Workstation  
**Status**: ACTIVE — MANDATORY BOUNDARY ENFORCEMENT  
...
- **Technical & Security Failure Modes**: 40–65% false-positive rate on raw HTTP; non-deterministic output breaks retesting; prompt injection vulnerabilities via hostile target HTTP responses; token cost exhaustion ($>1500\text{ms}$ latency per request).
- **Invariant Violations**: `SEC-06` (Finding Proof Requirement), `SEC-03` (Host-Side AI Policy Gate).
- **SENTINEL V6 Alternative**: Adaptive Test Planner (`sentinel_planner`) and 5-Tier Verification Engine (`sentinel_verification`). AI is strictly restricted to offline test vector synthesis behind an immutable host-side policy gate.
...
- **SENTINEL V6 Alternative**: Controlled Agentic Testing (`sentinel_agentic`) with strictly typed tools, enforced risk budgets, and mandatory human confirmation gates on destructive actions.
...
- **Technical & Security Failure Modes**: Hallucinated technical details, incorrect curl commands, and fabricated severity scores that fail legal admissibility.
...
- **Invariant Violations**: `SEC-01` (Scope Authorization - Default Deny).
- **SENTINEL V6 Alternative**: Scope-Constrained Depth-Bounded Crawler (`sentinel_discovery`) with Aho-Corasick Trie filtering.
...
- **SENTINEL V6 Alternative**: Adaptive Bayesian Test Planner (`sentinel_planner`).
...
- **Invariant Violations**: `SEC-06` (Deterministic Verification).
- **SENTINEL V6 Alternative**: Semantic AST Myers Diffing with dynamic entropy masking (`sentinel_diff`).
...
- **SENTINEL V6 Alternative**: Passive JS Bundle Route Extraction & OpenAPI Schema Introspection (`sentinel_discovery`).
...
- **SENTINEL V6 Alternative**: Verification-First CVE Engine (`sentinel_vulnintel`): CVE is reported only after executing an active, non-destructive verifying probe.
...
- **SENTINEL V6 Alternative**: Passive Token-Normalized State Transducer (`sentinel_state`).
...
- **SENTINEL V6 Alternative**: Context-Aware Parameter Dependency Extraction (`sentinel_discovery`).
...
- **SENTINEL V6 Alternative**: Sandboxed WebAssembly (WASM) Plugin Runtime with zero ambient capabilities and explicit host-mediated IPC gates (`sentinel_plugins`).
...
The rejection of these 25 anti-patterns constitutes the **Anti-Overengineering Boundary** of SENTINEL V6. Any future proposal attempting to reintroduce an approach cataloged in this register must be automatically rejected unless it mathematically disproves the identified failure modes.
